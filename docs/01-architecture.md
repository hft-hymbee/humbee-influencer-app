...
| Maps | **`react-native-maps`** — Google on Android, **Apple Maps on iOS** | The construction-site picker only parks a pin; nothing reads a place name off the map. Forcing Google on iOS would add the Google Maps iOS SDK and a second billed key for no gain, and Apple Maps renders on a simulator with no key at all. Android needs `HUMBEE_MAPS_API_KEY` — a blank key builds and renders a **grey tile grid**, which is the first symptom to check. ~3–5 MB against the 30 MB budget |
| Device location | **`@react-native-community/geolocation`**, wrapped in `src/platform/location.ts` | The ONLY file that touches GPS or a permission prompt — rules that live in one file stay rules. When-in-use only; `ACCESS_BACKGROUND_LOCATION` is absent from the manifest by design. Two-stage fix (GPS 8s → coarse 12s): high-accuracy alone times out indoors while the OS *has* a wifi/cell position it declines to hand over. `setRNConfiguration({ skipPermissionRequests: true })` stops the library firing its own prompt on top of ours |
