...
| 15h | ~~No forward place search~~ — **CLOSED, the endpoints exist.** `GET /address/search` (autocomplete → suggestions) and `GET /address/places/{place_id}` (a suggestion → the reverse-geocode shape + `place_id`/`place_name`). They are absent from `V2/01-api-reference.md` and documented instead in the backend repo at `logic/influencer/design_docs/influencer.md` §"Search" | **`V2/01-api-reference.md` needs both endpoints added** — a client built from V2 alone concludes, as this one did, that search does not exist. Built Sep 2026 against the live `/openapi.json`, which carries both |
| 15l | **`epin` and `fulfilment` on the demand row are absent from `V2/01-api-reference.md`.** Both are live — `epin` (`code`, `issued_at`, `attempts_remaining`, `verified_at`) and `fulfilment` (`status`, `status_label`, `demanded`, `allocated`, `remaining`) — and documented only in the backend repo's `design_docs/`. `fulfilment` directly contradicts V2's "a demand carries no status", which is why My Demands has no status chips | **V2 needs both added, and the "no status" statement withdrawn.** ePIN is built and rendered (Sep 2026). `fulfilment` is now rendered as the demand status chip (Sep 2026, at the client's direction), which reverses the no-status design. **The design spec's My Demands screen still shows no chip** — `docs/design-spec/04-screens/08-my-demands.md` needs updating to match, and the three fulfilment states are not the PRD's four-state chain |
| 15i | **No saved sites.** Spec R7 and S5's "YOUR SAVED SITES" section need `GET/POST /influencer/sites` (+ rename/delete). V2 has none, so a site is re-pinned from scratch every time | Screen 06 site capture. The spec calls repeat demand against the same site "the dominant case" — which makes this the single biggest tap-count win available on this screen. The S6 "Save this site for future demands" checkbox is **not built**, because a checkbox that saves nowhere is worse than no checkbox. NOT faked in MMKV: device-local "saved sites" would be lost on reinstall and invisible to Ops |
| 15j | **`site` on a demand row carries no `accuracy`, `source` or captured-at.** The app knows all three at capture time (GPS vs. placed pin, metres, when) and drops them at the submit boundary | Nothing is blocked. But the UI spec's §10 analytics (`site_location_source_used`, `site_pin_dragged`) and its §7 "raw geocode response, stored for audit" both assume the server keeps them. If demand-per-site quality is ever to be audited — how many sites were pinned accurately vs. dropped roughly — the fields have to exist on the wire first |
| 15k | **Spec/contract conflict on S6's three derived fields.** The UI spec shows Pincode, District and State as editable inputs ("a wrong reverse-geocode is correctable without leaving the screen"); the contract stores the state and district the **pincode** names and rejects contradicting ids outright with `SITE_ADDRESS_INVALID` | Screen 06. **Built read-only**, per the contract and per the spec's own note under those fields ("Change the pin to change them"). An editable field the server will refuse invites a correction that can only fail at submit, several taps later, with the cart already built. Confirm this is the intended reading — it is the spec's §13 open question 5 |
