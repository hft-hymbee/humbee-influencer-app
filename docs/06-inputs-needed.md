...
| 15h | **No forward place search.** V2 ships `GET /address/reverse-geocode` (pin → address) and nothing the other way, so the site flow's search screen (S5 of `construction-site-address-capture.md`) has a field, a current-location row and an empty state, but no results. The UI spec proposes `/geo/search?q=&lat=&lng=`, district-biased, capped at 8 | Screen 06 site capture. Today the only way to reach a distant site is to pan the map to it, which on a slow connection is the difference between a 10-second capture and a minute. **The key must stay server-side** — the spec is explicit that the app never calls a geocoding provider directly, so this cannot be worked around client-side |
